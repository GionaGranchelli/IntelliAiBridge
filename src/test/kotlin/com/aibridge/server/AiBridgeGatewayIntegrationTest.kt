...
class AiBridgeGatewayIntegrationTest {
    private val mapper = jacksonObjectMapper()
    private val http = HttpClient.newHttpClient()
    private lateinit var gateway: AiBridgeGateway
    private lateinit var settings: AiBridgeSettings
    private lateinit var baseUrl: String
    private val apiKey = "test-aibridge-key"

    @BeforeEach
    fun setUp() {
        settings = AiBridgeSettings().apply {
            host = "127.0.0.1"
            port = findFreePort()
            enableLogging = false
            autoStart = false
    ...
    fun `health endpoint responds without auth`() {
        ...
        assertEquals("ide", json["platform"].asText())
    }
