...
/**
 * Persistent application settings for the AiBridge gateway.
 *
 * Sensitive API key material is stored in [AiBridgeSecretStore] instead of
 * plain-text XML state.
 */
@State(
    name = "com.aibridge.settings.AiBridgeSettings",
    storages = [Storage("AiBridge.xml")]
)
@Service(Service.Level.APP)
class AiBridgeSettings : PersistentStateComponent<AiBridgeSettings> {
    /** AI providers supported by the bridge. */
    ...
    var activeProvider: AiProvider = AiProvider.COPILOT
    ...
    override fun getState(): AiBridgeSettings = this

    /**
     * Loads persisted state and migrates any legacy plaintext API key value
     * into Password Safe.
     */
    override fun loadState(state: AiBridgeSettings) {
        val legacyApiKey = state.apiKey
        XmlSerializerUtil.copyBean(state, this)
        if (legacyApiKey.isNotBlank() && AiBridgeSecretStore.getApiKey().isBlank()) {
            AiBridgeSecretStore.setApiKey(legacyApiKey)
        }
        // Prevent future plaintext persistence after migration.
        apiKey = ""
    }

    /** Returns API key currently stored in IDE Password Safe. */
    fun getStoredApiKey(): String = AiBridgeSecretStore.getApiKey()

    /** Persists API key in Password Safe and clears plaintext compatibility field. */
    fun setStoredApiKey(apiKey: String) {
        AiBridgeSecretStore.setApiKey(apiKey)
        // Keep persisted field empty to avoid plaintext storage in settings XML.
        this.apiKey = ""
    }

    /** Indicates whether `AIBRIDGE_API_KEY` is present in process environment. */
    val isEnvApiKeyConfigured: Boolean
        get() = !System.getenv("AIBRIDGE_API_KEY").isNullOrBlank()

    /** Describes which source currently supplies the effective API key. */
    val apiKeySource: ApiKeySource
        get() {
            val envApiKey = System.getenv("AIBRIDGE_API_KEY")
    ...
    val effectiveApiKey: String
        get() = System.getenv("AIBRIDGE_API_KEY")?.takeIf { it.isNotBlank() } ?: getStoredApiKey()

    companion object {
        /** Convenience accessor for application-level settings service. */
        val instance: AiBridgeSettings
            get() = ApplicationManager.getApplication().getService(AiBridgeSettings::class.java)
    }
}
