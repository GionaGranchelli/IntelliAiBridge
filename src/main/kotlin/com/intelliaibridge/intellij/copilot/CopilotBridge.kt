...
/**
 * Adapter over GitHub Copilot IntelliJ APIs used by IntelliAiBridge.
 *
 * It isolates plugin-specific session/model behavior from HTTP gateway logic.
 */
class CopilotBridge {
    /** Model descriptor exposed to IntelliAiBridge settings and `/v1/models`. */
    data class AvailableModel(
        val id: String,
        val label: String
    )

    /** Stream of normalized events emitted while Copilot processes a prompt. */
    sealed class Event(open val type: String) {
        data class Progress(val reply: String?, val kind: String?) : Event("Progress")
        data class Complete(val reply: String?) : Event("Complete")
        data class Error(val message: String) : Event("Error")
        data class Other(override val type: String) : Event(type)
    }
     ...
     */
    suspend fun prepareSession(project: Project, requestedModel: String?, defaultModel: String): SessionHandle {
    ...
    suspend fun sendMessage(handle: SessionHandle, prompt: String, onEvent: (Event) -> Unit) {
        val handler = object : AgentProgressHandler<CopilotAgentConversationProgressEvent> {
            override fun on(event: CopilotAgentConversationProgressEvent) {
                val mapped = when (event) {
                    is CopilotAgentConversationProgressEvent.Progress -> Event.Progress(event.progress.reply, event.progress.kind)
                    is CopilotAgentConversationProgressEvent.Complete -> Event.Complete(event.complete.reply)
                    is CopilotAgentConversationProgressEvent.Error -> Event.Error(event.error.message ?: "Unknown Copilot error")
                    else -> Event.Other(event.javaClass.simpleName)
                }
                onEvent(mapped)
            }
        }
        withContext(Dispatchers.EDT) {
            handle.controller.sendMessage(prompt, handle.dataContext, handler)
        }
    }
     ...
     */
    fun extractVisibleAssistantText(handle: SessionHandle): String? {
    ...
    suspend fun cleanupSession(handle: SessionHandle, log: (String) -> Unit) {
        try {
            handle.sessionManager.deleteSession(handle.session.id)
        } catch (e: Exception) {
            log("Failed to cleanup session ${handle.session.id}: ${e.message}")
        }
    }
    ...
    private fun mapCatalogModels(modelService: CompositeModelService): List<AvailableModel> {
        return modelService.models.unscoped.value
            .map { model ->
                ...
                val id = if (provider != null) "${model.modelName} ($provider)" else model.modelName
                AvailableModel(id = id, label = id)
            }
            .distinctBy { it.id }
